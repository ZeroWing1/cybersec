{* $Id: tiki-view_tracker_item.tpl,v 1.76.2.111 2008/04/04 17:01:41 kerrnel22 Exp $ *}
...
<td colspan="3">
{/if}
{if $cur_field.type eq 'f' or $cur_field.type eq 'F' or $cur_field.type eq 'j'}
{if $cur_field.value}{$cur_field.value|tiki_long_date}{/if}
...
{elseif $cur_field.type eq 'F'}
  {if $cur_field.isMandatory eq 'y'}
    {html_select_date prefix="ins_"|cat:$cur_field.id start_year="-4" end_year="+4" time=$cur_field.value}
  {else}
    {if $cur_field.options_array[0] eq '0' && $cur_field.value eq ''}
      {html_select_date prefix="ins_"|cat:$cur_field.id start_year="-4" end_year="+4" year_empty=" " month_empty=" " day_empty=" " time="--"}
    {else}
      {html_select_date prefix="ins_"|cat:$cur_field.id start_year="-4" end_year="+4" year_empty=" " month_empty=" " day_empty=" " time=$cur_field.value}
{$cur_field.value} :::
    {/if}
  {/if}
...
{elseif $cur_field.type eq 'r'}
<select name="ins_{$cur_field.id}">
{if $cur_field.isMandatory eq 'y'}<option value=""></option>{/if}
