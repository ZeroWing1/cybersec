{* $Header: /cvsroot/tikiwiki/tiki/templates/tiki-upload_file.tpl,v 1.12.10.12 2008/03/12 18:29:56 marclaporte Exp $ *}
		...
		{section name=ix loop=$uploads}
			<div align="center">
				...
				<div class="wikitext">
					...
					{tr}You can include the file in an Wiki page using{/tr}: <textarea cols="60" rows="2">[tiki-download_file.php?fileId={$uploads[ix].fileId}|{$uploads[ix].name} ({$uploads[ix].size|kbsize})]</textarea><br /><br />
					{tr}You can include the file in an HTML page using{/tr}: <textarea cols="60" rows="2">&lt;a href="{$url_browse}?fileId={$uploads[ix].fileId}"&gt;{$uploads[ix].name} ({$uploads[ix].size|kbsize})&lt;/a&gt;</textarea>
				</div>
			</div>
		{/section}
